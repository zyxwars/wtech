                ...
                <div class="w-full max-w-[1600px] px-4 lg:px-20">
                    ...
                    <section class="mb-4 flex w-full justify-end gap-2">
                        <select class="select select-primary max-w-42" onchange="this.form.submit()" name="order">
                            ...
                            <option value="price_asc" {{ request("order") == "price_asc" ? "selected" : "" }}>Price Lowest</option>
                            <option value="price_desc" {{ request("order") == "price_desc" ? "selected" : "" }}>Price Highest</option>
                        </select>
                    ...
                    <article class="product-grid mb-8">
                        ...
                        <article class="w-full">
                            ...
                            <div class="p-2">
                                <a href="{{ route('product', $product->id) }}" class="text-xl font-bold">
                                    <h3> {{ $product->title }}</h3>
                                </a>
        ...
        <div class="drawer-side z-20">
            ...
            <div
                ...
                <div class="flex gap-2">
                    <input name="price_start" value="{{ request("price_start") }}" type="number" placeholder="Start" class="input" min="0"/>
                    <input name="price_end" value="{{ request("price_end") }}" type="number" placeholder="End" class="input" />
                </div>
                ...
                <select name="author" class="select">
                    ...
                    <option value="{{ $author }}" {{ request("author") == "$author" ? "selected" : "" }}>{{ $author->name }}</option>
                    @endforeach
                </select>
                ...
                <select name="language" class="select">
                    ...
                    <option value="{{ $language }}" {{ request("language") == "$language" ? "selected" : "" }}>{{ $language->name }}</option>
                    @endforeach
                </select>

                <h2 class="mt-4 text-lg font-bold">Release year</h2>
                <div class="flex gap-2">
                    <input name="release_year_start" value="{{ request("release_year_start") }}" type="number" placeholder="Start" class="input" min="0"/>
                    <input name="release_year_end" value="{{ request("release_year_end") }}" type="number" placeholder="End" class="input" />
                </div>
